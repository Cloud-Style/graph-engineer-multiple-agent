...
class HeuristicLlmPort:
    ...
    def complete(self, prompt: str) -> str:
        ...
        if "STAGE=implement" in prompt:
            match = re.search(r"^MODULE=(.*)$", prompt, flags=re.MULTILINE)
            module = match.group(1).strip() if match else "app"
            rev_match = re.search(r"^REVISION=(\d+)$", prompt, flags=re.MULTILINE)
            revision = int(rev_match.group(1)) if rev_match else 1
            body = (
                f'"""Generated by macs for module {module}."""\n'
                f"MACS_IMPL_MARKER = {module!r}\n"
                f"REVISION = {revision}\n"
            )
            if revision > 1:
                body += 'RETRY_NOTE = "Retry after reviewer routing"\n'
            return json.dumps(
                {
                    "files": [
                        {
                            "path": f"{module}/{module}.py",
                            "content": body,
                        }
                    ]
                }
